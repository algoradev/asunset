...
**Week 1 — pilot.** Deploy; reconcile sees the new key, writes *zero* grants (D1). The consumer doctor dry-runs the reconcile and reports clean. Then the step the first draft glossed (kestrel's realism catch): **who** grants the feature to the Growth team? The credential that can is the *client's* — the first `platform_admin` came from their `POST /platform/bootstrap` and forced TOTP enrollment, not from the product vendor. v1.1's surface is API-only, and a client-org admin on day one does not curl authenticated endpoints — so the honest early answer is either the vendor's operator acting on the client's behalf (**a trust-boundary problem, not a UX one**) or the action waits for a surface the client admin can actually drive. The story's ruling: the grant is performed by the client's admin using a documented, token-custody-solved invocation (see the runbook-command DX item) — vendor staff reaching into a client's authz is the anti-pattern the boundary exists to prevent. One audited call, `feature.granted`, who-granted-what-when in the record. *(v1.1 surface; in v1 this step is the wall — why this feature class waits for v1.1 rather than anyone hand-writing a tuple.)*

**Week 3 — an incident.** Costs spike — a runaway prompt loop. On-call **freezes** the feature — and the realistic version of "one API call" (kestrel) is a **runbook command**, not a remembered curl: a documented `feature freeze <key>` invocation whose token custody was solved *before* the incident, because at 2am the failure mode is on-call without a valid token or a fat-fingered key while costs burn. Every check denies *now*; the grant ledger is intact, so "who had access during the window" is a query, not archaeology. Fixed Friday; **unfreeze**; access returns exactly as it was. Nobody edited a file mid-incident. *(v1.1 — and the endpoint and its invocation ship as ONE deliverable, never two.)*
...
**Added from kestrel's operator-chair review (2026-07-25):**

7. **The incident-freeze runbook command** — 2am DX, and safety rather than convenience: the freeze/unfreeze endpoint ships *with* a single documented invocation whose token custody is pre-solved. A freeze endpoint nobody can invoke correctly at 2am is a half-built incident story. **Ships in lockstep with the v1.1 freeze endpoint — one deliverable, never two.**
8. **`dry_run` auth reachability + diffable output** — the consumer doctor runs as an instance/service identity, not `platform_admin`; if `dry_run` inherits the mutating endpoint's auth bar, the whole verify-not-reimplement story collapses. `dry_run` is a read — it needs an auth bar the doctor's identity can clear (machine-credential path to be designed in v1.1), and its report must be deterministically ordered so drift is diffable and eyeball-able in a runbook.
9. **Point-in-time access-matrix export** — compliance DX: the Act-4 matrix generated from *live* state at audit time (feature → grant → who, with provenance), dated, as the artifact a HIPAA reviewer actually asks for. Rides v1.1's provenance listing.
