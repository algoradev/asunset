...
# Migrations run as the schema owner, not the app user. The app user
# is non-owner by design (see infra/postgres/init/01-databases.sh).
config.set_main_option("sqlalchemy.url", get_settings().app_admin_db_url)
