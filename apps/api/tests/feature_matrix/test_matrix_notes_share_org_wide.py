"""Matrix-row evidence for notes.share.org_wide."""
...
FEATURE_KEY = "notes.share.org_wide"
EXPECTED_FINGERPRINT = "2019fd9da324ec92"


def test_notes_share_org_wide_declaration_current() -> None:
    assert_declaration_fingerprint("features.yaml", FEATURE_KEY, EXPECTED_FINGERPRINT)


async def test_notes_share_org_wide_allowed_role_sharers_assignee() -> None:
    p = persona()
...
async def test_notes_share_org_wide_denied_outsider() -> None:
    assert await gate_status(FEATURE_KEY, StaticAuthorizer(), persona()) == 403


async def test_notes_share_org_wide_scope_is_shareable_notes() -> None:
    """Declared reach is share authority, not broad note visibility."""
    ...
    p = persona()
    authz = StaticAuthorizer()
    authz.allow(p.fga_user(), "can_delete", "note:org-ready")
